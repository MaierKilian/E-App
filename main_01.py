...
def render_questionnaire_tab(user_data: dict) -> None:
    ...
    st.markdown(
        ...
        <h2 class="screen-title">Grunddaten deiner Wohnung</h2>
        <p class="screen-copy">
            Je genauer diese Angaben sind, desto präziser werden Hinweise, Fortschritte und die Einschätzung deines Einsparpotentials ermittelt.
        </p>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        progress_bar_html("Profilstaerke", filled, total, "Basis für spaetere Hinweise und Einsparwerte"),
        unsafe_allow_html=True,
    )
...
def render_measurements_overview(user_data: dict) -> None:
    ...
    st.markdown(
        ...
        <p class="screen-copy">
            Jede Kachel steht für ein Gewerk. Warmwasser ist bereits klickbar und
            führt direkt zum Duschkopftest. Die anderen Kacheln bleiben vorerst Platzhalter.
        </p>
        """,
        unsafe_allow_html=True,
    )
...
def render_warmwater_detail(user_data: dict) -> None:
    ...
    if filled < total_fields:
        st.info(
            "Du kannst den Test schon starten. Die Einscätzung in Euro wird noch genauer, "
